...
prx.types.text = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '</style>';
		cR += prx.items.getComponentPrependDivs(item, containerid, symbol);
		cR += '<div class="text-contents '+(prx.componentsHelper.getProp(item.autoResize,'boolean')==true ? ' autoResize-true' : '')+ ' liveUpdate-backgroundColor changeProperty-backgroundColor" style="'+prx.gradients.getBgCssByProperty(item.backgroundColor,'color-background')+';">';
...
prx.types.placeholder = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '</style>';
		cR += prx.items.getComponentPrependDivs(item, containerid, symbol);
		cR += '<div class="bg liveUpdate-backgroundColor liveUpdate-borderColor changeProperty-backgroundColor changeProperty-borderColor" style="'+prx.gradients.getBgCssByProperty(item.backgroundColor,'color-background')+'; border-color: '+prx.componentsHelper.getProp(item.borderColor,'color-border')+'; border-width: '+prx.componentsHelper.getProp(item.thickness,'num-border-width')+'px;"></div>';
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		$.each(item.tabs, function(i,elm){
			...
			if(prx.componentsHelper.getProp(item.iconpos,'other') != "") {
				...
				cR += '</style>'
				cR += '<div class="icon-wrapper"><div class="icon '+ ((prx.componentsHelper.getProp(item.maskEnabled,'boolean') && $.browser.webkit) ? ((prx.componentsHelper.getProp(item.activeTab,'num-other') == i) ? 'liveUpdate-maskActive' : 'liveUpdate-maskInactive') : '') +'"></div>';
			}
